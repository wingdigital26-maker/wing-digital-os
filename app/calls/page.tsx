...
// The Call Room, one screen. This used to be split in two — a "Today" summary
// dashboard and a separate "Dial list" — which meant the numbers lived on one
// tab and the leads you actually work lived on another. They are now one thing:
// a compact today-strip (your calls/booked today + any call-backs owed right
// now) sits on top of the full, workable lead list. Page chrome (header, tabs,
// sign-out) belongs to app/calls/layout.tsx.
...
// The today-strip's shape, distilled from /api/calls/stats. Only the handful of
// numbers the merged screen shows on top of the list; the rest of the old
// dashboard (funnel, activity feed) is not duplicated here.
type Today = {
  calls: number;
  booked: number;
  callbacksDue: number;
...
export default function CallRoom() {
  ...
  // The today-strip. Distilled from /api/calls/stats — the same endpoint the
  // old Today dashboard used — down to the four numbers the merged screen
  // shows. Best-effort: a failure here never blocks the list, it just hides
  // the strip.
  const loadToday = useCallback(async () => {
    try {
      ...
      if (!r.ok) return;
      const d = await r.json();
      const callbacks: { overdue?: boolean }[] = Array.isArray(d?.callbacks) ? d.callbacks : [];
      setToday({
  ...
  return (
    <>
      <div>
        ...
        {/* today-strip: your day at a glance, folded in from the old Today tab.
            Two live numbers plus, when anything is owed, a loud call-back alert
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 16, alignItems: "stretch" }}>
          ...
          {today && today.callbacksDue > 0 && (
            <button
              ...
              title="Jump to the call-backs you owe"
              style={{
            ...
            >
              ...
              <span style={{ minWidth: 0 }}>
                <span style={{ display: "block", fontSize: 13.5, fontWeight: 700 }}>
                  {today.callbacksDue === 1 ? "call-back owed right now" : "call-backs owed right now"}
                </span>
                <span style={{ display: "block", fontSize: 12, color: "var(--text-muted)", marginTop: 1 }}>
                  {today.callbacksOverdue > 0
                    ? `${today.callbacksOverdue} overdue · tap to work them`
                    : "tap to work them"}
                </span>
              </span>
            </button>
