    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18, marginBottom: 28 }}>
      ...
      {queue.length > 0 && (
        <div style={{
          ...
          boxShadow: "0 8px 24px rgba(0,0,0,0.25), 0 0 24px rgba(251,191,36,0.06)",
        }}>
          <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>
            ⚠️ Needs Jack <span style={{ background: "rgba(251,191,36,0.2)", color: "#fbbf24", fontSize: 11, padding: "2px 10px", borderRadius: 999, marginLeft: 6 }}>{queue.length}</span>
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {queue.map(item => (
              <div key={item.id} style={{ background: "rgba(0,0,0,0.25)", borderRadius: 10, padding: "12px 14px", borderLeft: "3px solid #fbbf24" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    ...
                    {item.action && <p style={{ fontSize: 11, color: "#fbbf24", marginTop: 4 }}>Approving = {item.action}</p>}
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => resolve(item.id, "approved")} disabled={resolving === item.id}
                      style={{ padding: "6px 16px", borderRadius: 999, fontSize: 11.5, fontWeight: 700, cursor: "pointer", border: "1px solid rgba(52,211,153,0.5)", background: "rgba(52,211,153,0.12)", color: "#34d399" }}>
                      Approve ✓
                    </button>
                    <button onClick={() => resolve(item.id, "dismissed")} disabled={resolving === item.id}
                      style={{ padding: "6px 14px", borderRadius: 999, fontSize: 11.5, cursor: "pointer", border: "1px solid rgba(255,255,255,0.12)", background: "transparent", color: "var(--text-muted)" }}>
                      Dismiss
                    </button>
                  </div>
                </div>
