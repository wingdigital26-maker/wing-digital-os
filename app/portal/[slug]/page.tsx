...
// Client-facing labels: plain English, no internal shorthand.
const PILLARS: { key: keyof Health; label: string }[] = [
  { key: "seo", label: "Search visibility" },
  { key: "content", label: "Content" },
  { key: "website", label: "Website" },
  { key: "crm", label: "Leads & follow-up" },
  { key: "onboarding", label: "Setup" },
];

// Internal names arrive as machine text ("content_engine", "in_progress").
// Everything shown to a client gets cleaned up into readable words first.
...
function statusLabel(s: unknown): string {
  if (typeof s !== "string" || !s) return "";
  return STATUS_LABEL[s.toLowerCase()] ?? humanize(s);
}
    ...
    <div className="page-scroll" style={{ minHeight: "100vh", background: "var(--bg-primary)", color: "var(--text-primary)" }}>
      <header style={{ padding: "20px 28px", borderBottom: "1px solid var(--border)", background: "var(--bg-secondary)" }}>
        ...
        <div style={{ display: "flex", gap: 14, marginTop: 8, flexWrap: "wrap" }}>
          {client.phase && <span style={{ fontSize: 12, color: "var(--accent-2)" }}>Current phase: {humanize(client.phase)}</span>}
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* Deliverables */}
        <section>
          <p style={sectionTitle}>Work delivered</p>
          <div style={card}>
            {deliverables.length ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {deliverables.map((d: any, i: number) => (
                  <div key={d.id ?? i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < deliverables.length - 1 ? "1px solid var(--border)" : "none" }}>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>{d.title ?? d.name ?? (humanize(d.type) || "Deliverable")}</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {agentRuns.map((r: any, i: number) => (
                  <div key={r.id ?? i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < agentRuns.length - 1 ? "1px solid var(--border)" : "none" }}>
                    <span style={{ fontSize: 13 }}>{humanize(r.agent ?? r.name ?? r.type) || "Automated task"}</span>
                    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                      {r.status && <span style={{ fontSize: 11, color: r.status === "ok" ? "var(--green)" : "var(--text-muted)" }}>{statusLabel(r.status)}</span>}
                      {(r.created_at ?? r.date) && (
