...
export const ACTION_DEFS: Record<ActionType, { label: string; hint: string; fields: ActionConfigField[]; contacts_human: boolean }> = {
  ...
  send_email: { label: "Send an email", hint: "Drafted unless sending is switched on for this deployment", contacts_human: true, fields: [{ key: "subject", label: "Subject", kind: "text", required: true }, { key: "body", label: "Body", kind: "textarea", required: true }] },
